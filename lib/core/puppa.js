...
runTest = async function () {
    ...
    try {
        ...
        // detect not loaded iframe
        await felem(() => document.querySelector('#contentId').contentDocument.body.innerHTML, elem => {
            if(!elem.length) {
                // no markup in the iframe
                throw ("Page not loaded");
            } else if(elem.includes("main-frame-error")){
                // in case of network error chrome autogenerate markup with image 'Ups!' etc
                throw("Page not loaded");
            }
        },{pooling: 200, timeout: window.$$$config.browserConfig.timeout});
        ...
        // failure
        window.$$$result.error = typeof e === 'string' ? e : e.message;
