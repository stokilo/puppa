...
runTest = async function () {
    ...
    try {
        ...
        await felem(() => {
            let elem = [];
            try{
                elem = document.querySelector('#contentId').contentDocument.body.innerHTML;
                // if there is no content or page is rendered with chromium autogenerated markup (Ops page not laoaded)
                // that indicate network issue or problem with website.
                if(!elem.length || elem.includes("main-frame-error")) {
                    return [];
                }
            }catch(e) {
                return [];
            ...
            return elem;
        }, elem => true, {pooling: 200, timeout: window.$$$config.browserConfig.timeout, error: "Page not loaded (network or web site error)"});
        ...
        console.info("Test result: passed");
        console.dir(window.$$$result);
    } catch (e) {
        // failure
        ...
        console.info("Test result: failed");
        console.dir(window.$$$result);
    }
}
