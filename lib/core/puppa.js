 ...
 */
let elem = async function (selector, promiseHandle, options) {
    let options = options || {};
 ...
 * 
 * @param {*} selectorFunc function that performs element search on the page
 * @param {*} promiseHandle function to be called if element appear on the page, it gets element as a parameter
 * @param {*} options object with options 
 *            'timeout' 
 *                     (number of ms after which runner stop waiting for element and throws an error)
 ...
 */
let felem = async function (selectorFunc, promiseHandle, options) {
    let options = options || {};
    options.pooling = "pooling" in options && $.isNumeric(options.pooling) ? options.pooling : 1000;
    options.timeout = "timeout" in options && $.isNumeric(options.timeout) ? options.timeout : 60000;
    options.retry = "retry" in options ? options.retry : { retry: 0, wait: 0 };
...
runTest = async function () {
    ...
    try {
        ...
        let fn = window[window.$$$config.testName];
 ...
 */
let waitFor = async function (target, pooling, timeout, retry) {
    let fn = $.isFunction(target) ? target : () => $("#contentId").contents().find(target);
    let pooling_ = $.isNumeric(pooling) ? pooling : 1000;
    let timeout_ = $.isNumeric(timeout) ? timeout : 60000;


    let stat = await checkSelector(pooling_, timeout_, fn)
        .then(checkIsVisible);
 ...
 */
let checkSelector = (pooling, timeout, fn) => {
 ...
 * @param (pdfUrl} pdfUrl url to fetch pdf document from
 * @param (expectFnName) expectFnName function name that return string with expected pdf text content
 */
let expectPdfContent = async function (pdfUrl, expectFnName) {
    let callerId = new Error().stack.split("\n")[2].trim();
    let pdf = await PDFJS.getDocument(pdfUrl);
    let pdfInfo = pdf.pdfInfo;
    let pdfText = "";
