...
var elem = async function (selector, promiseHandle, options) {
    ...
    if ($.isFunction(promiseHandle)) {
        await waitFor(selector, options.pooling, options.timeout, options.retry).then(elem => promiseHandle(elem));
    } else {
...
var felem = async function (selectorFunc, promiseHandle, options) {
    ...
    if ($.isFunction(promiseHandle)) {
        await waitFor(selectorFunc, options.pooling, options.timeout, options.retry).then(elem => promiseHandle(elem));
    } else {
...
var waitFor = async function (target, pooling, timeout, retry) {
    ...
    if (!stat.found) {
        if(retry.retry <= 0) {
           throw new Error("Element not found: [" + target.toString() + "] " + callerId + " " + stack);
        } else {
...
function elementByRegex(tagName, regex) {
    return $("#contentId").contents().find(tagName).filter(function () { return regex.test($(this).text()); });
}
