 ...
 *            'repeatMs'
 *                     (delay between executing selector search to check if appear on the page)
 */
var elem = async function (selector, promiseHandle, options) {
    var options = options || {} ;
    options.repeatMs = "repeatMs" in options && $.isNumeric(options.repeatMs) ? options.repeatMs : 500;
    options.timeout = "timeout" in options && $.isNumeric(options.timeout) ? options.timeout : 5000;

    if ($.isFunction(promiseHandle)) {
        await waitFor(selector, options.repeatMs, options.timeout).then(elem => promiseHandle(elem));
    } else {
        await waitFor(selector, options.repeatMs, options.timeout).then(() => true);
    }
}
 ...
 *                     (number of ms after which runner stop waiting for element and throws an error)
 *            'repeatMs'
 *                     (delay between executing selector function to check if appear on the page)
 */
var felem = async function (selectorFunc, promiseHandle, options) {
    ...
    if ($.isFunction(promiseHandle)) {
        await waitFor(selectorFunc, options.repeatMs, options.timeout).then(elem => promiseHandle(elem));
    } else {
        await waitFor(selectorFunc, options.repeatMs, options.timeout).then(() => true);
    }
}
